...
int main(int argc, char** argv)
{
   ...
   VideoCapture cap;
	...
	userdata data;
	data.img=frame;
	setMouseCallback("detector...",mouseHandler,&data);
        if (frame.channels() == 4)
	...
	/*	FILE  *fp=NULL;
	        fp=fopen("/home/lab7/object-coordinates.txt","a+");
	         if(fp==NULL)
	           {
	    ...
	    printf("%s:%s:%d,%d\n",time_substr,labelstr,x_coordinate,y_coordinate);
	    fprintf(fp,"%s:%s:%d,%d\n",get_time(),labelstr,x_coordinate,y_coordinate);
